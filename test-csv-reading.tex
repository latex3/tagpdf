...
\begin{document}
\ExplSyntaxOn
\tl_new:N\l__tag_role_info_tmpa_tl
\tl_new:N\l__tag_role_info_tmpb_tl
\msg_new:nnn { tag } {role-parent-child} { The~relation~of~parent~'#1\l__tag_role_info_tmpa_tl'~and~child~'#2\l__tag_role_info_tmpb_tl'~is~#3}
% from tagpdf.dtx ...
...
\pdf_version_compare:NnTF < {2.0}
 {
   \cs_new_protected:Npn \tag_check_parent_child:nnN #1 #2 #3
     {
       \tl_set:Nn \l__tag_role_info_tmpa_tl {~(#1)}
       \tl_set:Nn \l__tag_role_info_tmpa_tl {~(#2)}
 ...
 }
 {
   \cs_new_protected:Npn \tag_check_parent_child:nnN #1 #2 #3
     { 
       \tl_set:Nn \l__tag_role_info_tmpa_tl {~(#1)}
       \tl_set:Nn \l__tag_role_info_tmpb_tl {~(#2)}
%    \end{macrocode}
