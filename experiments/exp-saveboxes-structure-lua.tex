...
\newcommand\updateattribute[1]{
 \directlua{
   ...
   update_attribute(tex.getbox(#1),mc,type)}}
\ExplSyntaxOff
...
function update_attribute (head,mc,type)
    for n in node.traverse(head) do
      node.set_attribute(n,luatexbase.attributes.g__tag_mc_cnt_attr,mc)
      node.set_attribute(n,luatexbase.attributes.g__tag_mc_type_attr,type)
        if n.id == hlist_id or n.id == vlist_id then
            update_attribute (n.list,mc,type)
        end
    end
    return head
end
...
Die\setbox0\hbox{gelbe\box2} Ente

Das \updateattribute{0} \copy0\ Haus

Die \emph{\updateattribute{0}\copy0 xxx}\ Sonne
\end{document}
