...
\def\newsavebox#1{\@ifdefinable{#1}{\tl_new:c{l__tag_box_\cs_to_str:N#1_state_tl}{\newbox#1}}}

\AddToHookWithArguments{cmd/sbox/before}
  {
   \tl_set:cx {l__tag_box_\cs_to_str:N#1_state_tl}{\the\value{g__tag_struct_abs_int}}
  }
\AddToHookWithArguments {cmd/sbox/after}
  {
   \int_compare:nNnTF { \value{g__tag_struct_abs_int} } = {\tl_use:c{l__tag_box_\cs_to_str:N#1_state_tl}}
     {
       \tl_set:cn {l__tag_box_\cs_to_str:N#1_state_tl} {0}
     }
     {
       \tl_set:cn {l__tag_box_\cs_to_str:N#1 _state_tl} {1}
     }  
  }
...
\sys_if_engine_luatex:TF
 {
   \prg_new_conditional:Npnn \tag_if_box_tagged:N #1 {p,T,F,TF} 
    {
      \tl_if_exist:cTF {l__tag_box_\cs_to_str:N#1_state_tl}
       { 
         \int_compare:nNnTF {\tl_use:c{l__tag_box_\cs_to_str:N#1_state_tl}}={1}
          { \prg_return_true:  }
          { \prg_return_false: }      
       }
       {
         \lua_now:e
          {
            local~tagged=ltx.__tag.func.check_box_tagging 
              (tex.getbox(\int_use:N #1))
            if~tagged~then~
            print("XXXXXXXXXXXX")
              tex.print(token.get_macro("prg_return_true:"))
            else~
              tex.print(token.get_macro("prg_return_false:"))
            end
          }
       }
    }  
...
\newcommand\testtagged[1]{
 \directlua{
      tagged=ltx.__tag.func.check_box_tagging (tex.getbox(#1))
      print("#1", tagged)
   }}   
...
%\setbox2\hbox{blaue} 
%
Die\savebox\mybox{gelb rote} Ente

\newbox\myboxb
\setbox\myboxb\hbox{\teststructure{blub}}
...
\ExplSyntaxOn
\tag_if_box_tagged:NTF \myboxb {1yes}{NO}

% \bool_if:nTF{ \tag_if_box_tagged_p:N 
%\myboxb} {yes}{NO} 
\ExplSyntaxOff
\end{document}
%
