...
\DeclareRobustCommand\sbox[2]{%
   \tag_mc_end_push:
   \tag_struct_begin:n{tag=NonStruct,stash}
   \tl_set:Nx\l__tag_tmpa_tl{\tag_get:n{struct_num}}
   \tl_set:cx 
     {l_tag_box_\int_use:N#1_tl}
     {\int_eval:n{\tag_get:n{struct_counter}+\tag_get:n{mc_counter}}}     
   \setbox#1\hbox{%
%    we must change the minipage/parbox code to get a balanced structure   
     \cs_set_protected:Npn\@kernel@before@minipage{\tag_struct_begin:n{tag=\l__ltboxes_tag_tl}}
     \cs_set_protected:Npn\@kernel@after@minipage{\tag_struct_end:}
     \cs_set_protected:Npn\@kernel@before@parbox{\tag_struct_begin:n{tag=\l__ltboxes_tag_tl}}
     \cs_set_protected:Npn\@kernel@after@parbox{\tag_struct_end:}     
     \color@setgroup#2\color@endgroup}
   \tl_set:cx {l_tag_box_\int_use:N#1_tl}
     {
      \int_eval:n{\tag_get:n{struct_counter}+\tag_get:n{mc_counter}
       -\tl_use:c{l_tag_box_\int_use:N#1_tl}}
     }    
   \tag_if_box_tagged:NTF #1 
      { \tl_set:cx {l_tag_box_\int_use:N#1_tl}{\l__tag_tmpa_tl}}
      { \tl_set:cn {l_tag_box_\int_use:N#1_tl}{0}}
    \tag_struct_end: 
   }
  
\DeclareRobustCommand\usebox[1]
 {
  \leavevmode
  \tag_if_box_tagged:NTF #1
   {
     \tag_mc_end_push:
     \tag_struct_use_num:n {\tl_use:c { l_tag_box_\int_use:N#1_tl }}
     \copy #1\relax
     \tag_mc_begin_pop:n{}
   }
   {\tag_mc_reset_box:N #1 \copy #1\relax}
 }
...
%\savebox\testboxA{yellow}
%
%
%The \usebox\testboxA\ duck.
%
%The \usebox\testboxA\ sun.
%
%\savebox\testboxA{blue}
%
%The \usebox\testboxA\ duck.
%
%The \usebox\testboxA\ sun.

\savebox\testboxB{\parbox{3cm}{\begin{itemize}\item bear 
\item duck\end{itemize}}} 
