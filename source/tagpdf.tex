...
\subsection{Proof of concept: Tagging the documentation itself}
\TagP^With version 0.6 the documentation itself has been tagged. The tagging is in now way perfect. The validator from Adobe doesn't complain, but PAX3 wants alternative Text for all links. And as mentioned above: passing a formal test doesn't mean that the content is really good and usable. I have a lot doubts that the code parts are really readable. The bibliography and the references must be improved. The user commands used for the tagging and also some of the patches used are rather crude. So there is lot space for improvement.\TagPend

\begin{tcolorbox}
\TagP^Be aware that to tag the links a very experimental package and a experimental hyperref driver was needed. This files are not in the package!\TagPend
\end{tcolorbox}
...
\TagP^To connect such a link with the text a specific structure has to be added \TagPend
...
\TagP^Structure Element can have so-called attributes. A single attribute is a dictionary%\footnote{or a stream but this is currently not supported by the package as I don't know an use-case}
