...
As I got now the specification and all engines can set the version to 2.0
I will start to check to look into it too, but it should be noted that PAC3 doesn't like 2.0
at all, this makes testing a bit of a challenge.
...
\item
 ...
 e.g. if a chunk is opened on one page but closed on the next page.
...
\item[\PrintKeyName{paratagging-show}] Boolean. This activate/deactivates small red numbers in the places where the hook code is used.
...
Block 3 will here get the same id number as Block 1 and be stored in the structure \emph{before} Block 2. The \PDF{} will record the chunks in the order \texttt{Block 1 -- Block 3 -- Block 2}.
...
The question is here is how to add this to the hyperref commands without doing a lot patching? This had lead to a new project moves some definitions for links to the \LaTeX-kernel so that package likes hyperref and tagpdf can coexist and both add their code to links at the same time. This new code is now available in a test package
\pkg{pdfmanagment-testphase} and it allows to add the code through hooks to links. This code is
now added automatically to URI and GoTo links if tagging is activated.
...
options of \cs{tagpdfsetup} or with the two commands above. \emph{This is very experimental!}
...
\begin{enumerate}
\item The \emph{standard structure namespace for PDF 1.7}, also called the \emph{default standard structure namespace}. These are the structure names from PDF 1.7:
\ExplSyntaxOn
\clist_use:Nn \c_@@_role_sttags_pdf_pdfII_clist {,\c_space_tl} 
,~ \clist_use:Nn\c_@@_role_sttags_only_pdf_clist {,\c_space_tl}
\ExplSyntaxOff 
...
\ExplSyntaxOn
\clist_use:Nn \c_@@_role_sttags_only_pdfII_clist {,\c_space_tl}
\ExplSyntaxOff

\item MathML 3.0 as an \emph{other namespaces}. There are nearly 200 types in this name space, so I refrain from listing them here.
\end{enumerate}
  
To accomodate to this more complicated setup the syntax of the \texttt{add-new-tag} key has been extended.
It now takes as argument a key-value list with the following key:

\begin{description}
\item[\PrintKeyName{tag}] This is the name of the new type as it should then be used in |\tagstructbegin|.
\item[\PrintKeyName{namespace}] This is the namespace of the new type. The value should be a shorthand of a namespace. The allowed values are currently |pdf|, |pdf2|, |mathml| and |user|. The default value (and recommended value for a new tag) is |user|. 
