...
I'm sure that tagpdf still has bugs. Bug reports, suggestions and comments 
...
current version of the \pkg{pdfmanagment} package is needed. 
\end{tcolorbox}
...
changed: The value \texttt{false} disables the automatic tagging (the resulting tagging is then rather unpredicatable and depends on the surrounding page), the
...
\begin{description}
  ...
  \item[\PrintKeyName{title-o}] This key allows to set the dictionary
   ...
   \item[\PrintKeyName{root-AF}] This key allows to reference an
     associated file in the root structure element. The use of the key is no longer usefull as ngpdf \cite{ngpdf} will change its implementation and use files attached to the Catalog. 
...
\texttt{ref\_dest\_parent})---this also works if hyperref has not been loaded---and through
