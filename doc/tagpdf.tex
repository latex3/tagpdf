...
% To compile an *untagged* documentation, replace tagging=on by 
% tagging=of in the following \DocumentMetadata command. 

\DocumentMetadata
 {
   % change the following line to tagging=off to compile an untagged documentation
   tagging=on,
   tagging-setup={debug/parent-child-check=atend,tabsorder=structure},
   pdfversion=2.0,lang=en,pdfstandard=a-4f,pdfstandard=ua-2,
   %uncompress
 }
\DebugBlocksOff
\makeatletter
\def\UlrikeFischer@package@version{1.0g}
\def\UlrikeFischer@package@date{2026-09-23}
\makeatother
...
This package is not meant for direct use in document preambles. It is loaded automatically 
if \cs{DocumentMetadata} is used. 

The package started in 2018 as a tool to \emph{research} tagging. It is now the base of the code developed 
in the \pkg{latex-lab} bundle for the Tagged PDF project (i.e., loaded by that code)
\url{https://latex3.github.io/tagging-project/}. 

The package is developed and improved in parallel with the \LaTeX{} kernel, the code in 
the \pkg{latex-lab} bundle (part of the core \LaTeX{} distribution),  the \pkg{pdfmanagement} 
...
You need some knowledge about \TeX, \PDF{} and perhaps even Lua to use its commands
directly.

\medskip

Issues, comments and suggestions can be added as issues to these two github trackers:
...
The code has been written so that it can be added as a module to the \LaTeX{} 
kernel itself if it turned out to be usable. It therefore avoids patching 
...
adds patches everywhere and would need constant maintenance---one only needs 
...
management, the new para hooks allow to automatically tag paragraphs, after 
...
in the latex-lab bundle and are loaded through the \texttt{tagging} keys. 

I'm sure that \pkg{tagpdf} still has bugs. Bug reports, suggestions and comments 
...
but requires fewer compilations. pdflatex works ok and can be used for legacy 
...
that will be usable for larger documents. \PDF{} is a page oriented format 
...
The package uses quite a lot of labels (in generic mode more than with luamode). 
...
\pkg{rerunfilecheck} package as an intermediate work-around and when using 
pdflatex compile at least once or twice more often than normal. 
...
In 2018 \PDF{} 2.0 was released. The reference can now be bought at no 
...
\LaTeX{} therefore targets \PDF{} 2.0 and \pkg{tagpdf} has support for 
associated files, for name spaces and other \PDF{} 2.0 features.

\PDF{}~2.0 features are currently (beginning of 2025) still not well supported by 
\PDF~consumers, but some progress has been made. Foxit can handle MathML associated files
and to some extent MathML structure elements and,
...
\item
 ...
 It is rather easy to create a broken \PDF{}:
...
Other options (only for PDF 1.7 and older) are preflight of the (non-free) Adobe Acrobat and the free \PDF{} Accessibility Checker (PAC~2026) \parencite{pac2026}. 
A quite useful tool 
...
It needs a screen reader, users which actually know how to handle them, 
can test documents and can report where a \PDF{} has real accessibility problems.


\minisec{Validation woes}

Sadly validators can not always be trusted. 

At first lots of the requirements for PDF/UA-1 and PDF/UA-2 are human checks.
This means that a validator either doesn't check it or has to use some heuristic.

At second \LaTeX{} creates by default PDF 2.0 and not many validators handle that correctly. 
...
so that an uncompressed \PDF{} is created and the internal objects and 
...
tagging wasn't (and isn't) in any way perfect. The validator from Adobe didn't 
...
the improvements. The documentation now uses \PDF~2.0  and while the newest 
...
patches used are still rather crude. So there is lot of space for improvement. 

\begin{tcolorbox}[]
Be aware that to create the tagged version a current lualatex-dev and a 
current version of the \pkg{pdfmanagement} package is needed. 
\end{tcolorbox}
...
about the \texttt{Lbl} use}\label{fig:adobe}
\end{figure}


\section{Loading}

The package requires the new PDF management. With a current \LaTeX{} (2025-06-01 or newer)
the PDF management is loaded if you use the \cs{DocumentMetadata} command before \cs{documentclass}.
The \pkg{tagpdf} package can then be loaded and activated by using the \texttt{tagging=on} key. The exact behavior of
the \texttt{tagging} key is documented in \texttt{documentmetadata-support-doc.pdf} which
is part of the \pkg{latex-lab} bundle.

Various parts of the code differentiate between \PDF{} version 2.0 and lower versions. By default \cs{DocumentMetadata} will now set the \PDF{} version to 2.0.
If \PDF{} 1.7 is wanted it is required to set the version early in the first \cs{DocumentMetadata}
...
When loading \pkg{tagpdf} through the \texttt{tagging=on} keys, it is automatically activated. 
To deactivate it while still retaining all the other new code from the latex-lab files, you can use the \texttt{tagging=off} key. 

Starting with \LaTeX{} 2025-11-01, you can also use |\DocumentMetadata{}|, which will
then do the same as |\DocumentMetadata{tagging=off}|. This also deactivates the interword space code.

\minisec{Loading as package needs activation!}

It is not recommended anymore, but the package can also be loaded
normally with |\usepackage| or |\RequirePackage|. But it still requires 
the \PDF\ management, so you need to do something like this:
\begin{lstlisting}
\RequirePackage{pdfmanagement}
\RequirePackage{tagpdf}
\end{lstlisting}

But it will then -- apart from loading more packages and defining a lot of things
-- not do much. You will have to \emph{activate} it
with \verb+\tagpdfsetup+.

The \PDF\ management will in any
...
\begin{docCommand}{tag_if_active:TF}{\marg{true code}\marg{false code}}\end{docCommand}
...
Starting with version 0.99m the mode is set automatically (Lua mode for luatex, generic mode otherwise). The package options do nothing anymore and will be removed in future versions. 
...
This command sets up the general behavior of the package.
The command should normally be used only in the preamble
(for a few keys it could also make sense to change them in the document).

The key-val list understands at least the following keys. More keys are defined in some of the latex-lab modules; see table~\ref{tab:setupkey} for an overview which also includes older, now deprecated names.

The command is deactivated before the class is loaded if the key |tagging=off| is used in \cs{DocumentMetadata} and then does nothing. 
...
\item [\PrintKeyName{activate}] Like |activate/all|,
  \emph{additionally} opens at begin document a structure with
...
\item[\PrintKeyName{activate/struct-dest}] Boolean, initially true.
  ...
  \pkg{tagpdf} will automatically create structure destinations (see
...
\item[\PrintKeyName{activate/tree}] Boolean, initially
  ...
  the two other keys have been used too.
...
\item[\PrintKeyName{activate/spaces}] Boolean. The key
  ...
  least \PrintKeyName{activate/mc} has been used. It is on by default if you use 
  |tagging=on| in \cs{DocumentMetadata}. The |tagging=off| key of \cs{DocumentMetadata} deactivates it. 
  The old name of the key |interwordspace| is still supported but deprecated.

\item[\PrintKeyName{activate/softhyphen}] luamode only.
  The key activates and controls the replacing of hard hyphens from hyphenation
  by soft hyphens. It knows the values 
  \begin{itemize}
  \item \texttt{false}, \texttt{off} Deactivates the automatic handling. 
  \item \texttt{char} Replaces the hyphens by U+00AD if the font contains this glyph.
  Note that in some newer fonts U+00AD is an invisible character and so this could led to disappearing hyphens. 
  \item \texttt{artifact} This keeps the hard hyphen U+002D but
   surrounds it by an Artifact-MC if tagging. This only works if the document is tagged. It requires that the \PrintKeyName{active/tagunmarked} key is not set to false.
  \item \texttt{true}, \texttt{on} Currently does the same as \texttt{artifact}.
 \end{itemize}
 
  If tagging is not active, the softhyphen handling is not activated by default and the hyphen stays a hard-hyphen. It is possible to use the \texttt{char} mode with 
  \begin{verbatim}
  \DocumentMetadata{tagging=off,tagging-setup={activate/softhyphen=char},}
  \end{verbatim}
  As \cs{tagpdfsetup} is deactivated in the document is not possible to 
  change the mode locally in the document with it. 
  
  If tagging is active the default is the \texttt{artifact} mode. The mode can be changed with \cs{tagpdfsetup} at any time. Switching on and off the softhyphen is a global operation, a switch between artifact and char more is a local operation. 
...
\item[\PrintKeyName{role/new-attribute*}] Normally attributes declared with the previous key are only written into the PDF if used in a structure. This starred version of the key ensures that the attribute is always written to the PDF.
...
\item[\PrintKeyName{para/tag}] String. This key changes the second tag
  ...
  structure \enquote{without meaning}. 
...
is set in the PDF Catalog (which is normally the case if \pkg{hyperref} is used) 
...
\item[\PrintKeyName{debug/uncompress}] Sets both the \PDF{} compresslevel
  ...
  \PDF{}. Not really useful anymore as this can also 
  be set in \cs{DocumentMetadata}.

\item[\PrintKeyName{debug}] This key knows a number of sub-keys to
  ...
  \PrintKeyName{vvv}, \PrintKeyName{all}.  Sets the log level.
  Changing the value affects currently mostly the luamode:
  \enquote{higher} values give more messages in the log. The current
  levels and messages have been set up in a quite ad-hoc manner and
...
These commands are deprecated. Tagging sockets (documented in source2e) should be used instead. 
...
there is actually no guarantee that the stream contains the text in the order 
it should be read.
...
\subsection{Tag names}\label{sec:tagnames}
In the following there will be many references to \enquote{tag names}. 
In the PDF these are \enquote{PDF names}: strings with a slash at the begin like \texttt{/LBody} and \texttt{/Em}.
In the commands of \pkg{tagpdf} they are -- with some exceptions for commands meant for developers -- input without the slash. 

Tag names are used a lot and to avoid to slow down the compilation they are not purified and checked. They are also normally not passed through some escaping command, but there can be code parts where it happens anyway. It lies in the responsibility of the user to use sensible names that do not break under such conditions.

The value should therefore be a string that can be used as a PDF name (by adding a slash at the begin) and that is not changed if it is passed through a command that escapes
names. A good test is to use \verb+\pdf_name_from_unicode_e:n+: if this only adds a slash the name should be fine. Some examples:\\[1ex]   
    \ExplSyntaxOn
    \tagpdfsetup{table/header-rows=1}
    \begin{tabular}{>{\ttfamily}l>{\ttfamily}ll}
    tag~name      & escaped~with~\verb+\pdf_name_from_unicode_e:n+ & comment \\\midrule
    chapter       & \pdf_name_from_unicode_e:n{chapter} & good\\
    chapter-title & \pdf_name_from_unicode_e:n{chapter-title}& good\\
    chapter.title & \pdf_name_from_unicode_e:n{chapter.title}&good\\
    chapter~title &\pdf_name_from_unicode_e:n{chapter~title} & bad\\
    chapter/title &\pdf_name_from_unicode_e:n{chapter/title} &bad\\
    chapter(title)&\pdf_name_from_unicode_e:n{chapter(title)}& bad\\
    grüße         &\pdf_name_from_unicode_e:n{grüße}         & bad\\
    \end{tabular} 
    \ExplSyntaxOff
...
\item
 ...
 Also in a standard document a lot of text is created automatically, e.g. 
...
\item[Artifacts] They are marked with of a pair of keywords, \texttt{BMC} 
    ...
    prefix argument, the fixed tag name \texttt{/Artifact}. Artifacts should 
...
also used as an index in an array there shouldn't be \enquote{holes} in the 
numbering system. (It is perhaps possible to handle a numbering scheme not 
starting with 0 and having holes, but it will enlarge the \PDF{} as one would 
need dummy objects.)
...
\begin{enumerate}[begin-penalty=10000]
...
\end{lstlisting}
  ...
  \item There exist environments which process their content more than once 
      -- examples are \texttt{align} and \texttt{tabularx}. 
       So one has to check for duplicates and holes in the counting system.
...
\begin{description}
  ...
  \item[\PrintKeyName{stash}] Normally marked content will be stored in the 
      ...
      example you may perhaps want to put a margin note behind or before the 
...
\item[\PrintKeyName{lang}] This (experimental) key allows to add a \texttt{/Lang} entry on the MC. This probably is not conforming to the PDF specification but it works in Adobe reader. The value is a BCP-string, e.g. \texttt{en} or \texttt{de}. 
    The key is not so useful as you typically want to the set `/Lang` entry on all MC inside a region or structure. Once the requirements of the PDF specification has been clarified the interface will be extended. 
...
\begin{docCommand}{tag_mc_reset_box:N}{\meta{box}}\end{docCommand}
...
\item  \verb+struct_tag+: the tag name of the current structure (without role-mapping).
...
\item \verb+mc_counter+: This returns a number and works also if only
  ...
  mc-commands.
\item \verb+current_Sect+: (defined in latex-lab-sec) gives back the number of current Sect structure, it is, e.g., used by the footnote code.  
\end{itemize}              

\begin{docCommand}{tag_get:nN}{\marg{key word}\marg{tl var}}\end{docCommand}

This (not expandable) command returns the values of some variables and stores them into the tl var. Currently, the working key words are 
\begin{itemize} 
\item  \verb+struct_tag+: the tag name of the current structure (without role-mapping).

\item \verb+struct_num+: This returns a number.
\end{itemize}

\begin{docCommand}{tag_get:nnN}{\marg{number} \marg{key word}\marg{tl var}}\end{docCommand}
This (not expandable) command returns the values of some variables of the structure \marg{number} and stores them into the tl var. Currently, the working key words are 
\begin{itemize} 
\item  \verb+struct_C+: the attribute classes as a comma list. 
\end{itemize}
...
For now \pkg{tagpdf} added some first support for automatically tagging:
Starting with version 0.92 header and footer are by default automatically marked up as (simple) artifacts.

With the key \PrintKeyName{page/exclude-header-footer} the behavior can be
changed: The value \texttt{false} disables the automatic tagging (the resulting tagging is then rather unpredictable and depends on the surrounding page), the
value \texttt{pagination} adds additionally an \texttt{/Artifact}
structure with the attribute \texttt{/Pagination}, but the contents is still not tagged.

The (experimental) key \PrintKeyName{page/tag-header-footer} surrounds the header with an \texttt{/Artifact} structure too but then reenables tagging. It currently only works if the header and footer contain a \cs{parbox}, so if e.g. the \pkg{fancyhdr} package is used.  


If some additional markup (or even a structure) is wanted, something like this should be used (together with \PrintKeyName{page/exclude-header-footer} set to false) (here with
...
with the \pkg{l3pdffield} package will be handled by this
package.
...
at the begin and end of paragraphs. The tagging code makes use of these hooks to 
...
The approach we take (starting with march 2023) to resolve 
this is to model such \enquote{semantic} paragraphs with a structure named 
\texttt{\UseStructureName{para/semantic}} and use \texttt{P} 
(under the name \texttt{\UseStructureName{para/textblock}}) only for 
...
this can be found in the documentation of \texttt{latex-lab-block}. 

As a consequence the tagging code adds two structures if paratagging is 
activated. The code to tag display blocks extends this code to handle the 
...
outer level with the |\tagpdfsetup| key |para/tagging| and |para-flattened| 
or with the commands |\tagpdfparaOn|  and |\tagpdfparaOff| or by changing the plugs
of the tagging sockets -- they are defined and described in source2e, section lttagging. 
...
begin hook. In such cases it can be easier to use the |para/tag| key with the 
...
structure elements or -- as leaves of the tree -- the marked contents chunks 
...
\begin{description}
  ...
  \item[\PrintKeyName{actualtext}] This key inserts an \texttt{/ActualText} 
   ...
   \item[\PrintKeyName{attribute}] This key takes as argument a comma
    ...
    The key can be used more than once and works then additively: new attributes are added to the existing once. See the next key if you want
    to overwrite existing attributes.
      
    See also section~\ref{sec:attributes}.
  
  \item[\PrintKeyName{attribute*}] This key first clears the attributes
  and then works like \PrintKeyName{attribute}. This will be rarely needed
  but is offered in case that additional settings for a structure are passed  through a socket.
  
  \item[\PrintKeyName{attribute-unnamed}] This takes a list of attributes
  contents (the part which is set in the second argument of \texttt{role/new-attribute}). Note that unlike \PrintKeyName{attribute} it can not reuse attribute objects, every use creates a new object. 
  The argument is not expanded, but expansion can be forced (as with all keys) with \verb+attribute-unnamed:o+ or \verb+attribute-unnamed:e+
 
  \item[\PrintKeyName{attribute-unnamed*}] This clears the attributes and
  then works like \PrintKeyName{attribute-unnamed}
  ...
  \item[\PrintKeyName{title-o}] This key allows to set the dictionary
   ...
   1.7, but the code currently doesn't check the \PDF{} version, it is your 
   ...
   \item[\PrintKeyName{root-AF}] This key allows to reference an
     associated file in the root structure element. The use of the key is no longer useful as ngpdf \cite{ngpdf} will change its implementation and use files attached to the Catalog. 
 
   \item[\PrintKeyName{root-supplemental-file}] This is a variant of the previous key. It takes as argument a file name. It then embeds this file with \texttt{/AFRelationship /Supplement} and appends it as associated file to the structure root. This key is like \texttt{root-AF} no longer recommended. Use the following key instead.

 \item[\PrintKeyName{catalog-supplemental-file}] This key should be used instead of \texttt{root-supplemental-file}. It takes as argument a file name. It then embeds this file with \texttt{/AFRelationship /Supplement} and appends it as associated file to the Catalog. ngpdf \cite{ngpdf} will store a \texttt{.css} attached in this way and reference it in the head of the html. If a \texttt{html} is attached in this way, ngpdf will copy the content into the head of the derived html. This means that the content of such an html file should normally be some html snippet suitable for the head, e.g. some css-code inside \texttt{<style>} tags.       
...
\begin{docCommands}
 {  
  {doc name=tag_struct_gput:nnn,doc parameter=\marg{structure number}\marg{keyword}\marg{value}},
 }\end{docCommands}

The allowed \meta{keywords} are \texttt{ref}, \texttt{ref\_label} \texttt{ref\_dest}, 
\texttt{ref\_dest\_parent},\texttt{ref\_num} and \texttt{attribute}.

 
This command allows to add the value, the target structure of the \texttt{Ref} key, with four methods: directly as object reference (key word \texttt{ref}), through a label name set with the |label| key (key word \texttt{ref\_label}), through a destination name if a \cs{MakeLinkTarget} has been used in 
the target structure (key words \texttt{ref\_dest} and
\texttt{ref\_dest\_parent})---this also works if \pkg{hyperref} has not been loaded---and through
...
with the key \texttt{role/new-attribute} or with the command \verb+\tag_attr_new:nn+. Both take two arguments, a name and 
...
\subsubsection{Checking the structure}

\begin{docCommands}
 {
  {doc name=tag_if_in:nTF,doc parameter=\marg{structure tag}},
 }\end{docCommands}

With this command it is possible to test if a structure with tag \marg{structure tag} is a (direct or indirect) parent. \marg{structure tag} must be a standard structure type, no role mapping is applied. 

\begin{docCommands}
 {
  {doc name=tag_if_struct_exist:nTF,doc parameter=\marg{structure number}},
 }\end{docCommands}

With this command it is possible to test if a structure with number \marg{structure number} exists.
...
tagging (e.g. with the package \pkg{accsupp}). If the \PDF{} reader support
this (Adobe Reader does, Sumatra does not) one can change with it how a
piece of text is copied and pasted e.g. to split up a ligature.

\PrintKeyName{/Alt} (inserted by \pkg{tagpdf} with
...
with both keys and text-to-speech software (the built-in of Adobe and
NVDA).  To sum them up:
...
\item[\PrintKeyName{tag}] This is the name of the new type as it
  should then be used in \cs{tagstructbegin}. See section \ref{sec:tagnames} regarding the syntax of the value. 
...
\item[\PrintKeyName{role}] This is the type the tag should be mapped
  ...
  you should be aware that \pkg{tagpdf} can't (or more precisely won't)
...
By default \pkg{tagpdf} creates an random URI for the user namespace but it is possible to change that:
...
misses the \texttt{math} tag and \texttt{mathml} tags).
...
Package tagpdf Info: Checking Parent-Child 'pdf2:Document' --> 'latex:text-unit'
Package tagpdf Info: Parent-Child 'Document' --> 'Part'.
(tagpdf)             Relation is 1 (='0..n') on line ...

Package tagpdf Warning: Parent-Child 'pdf2:Document' --> 'pdf2:Ruby'.
(tagpdf)                Relation is not allowed! on line ...
(tagpdf)                struct 67, text-unit --> struct 68, Ruby
\end{lstlisting}
...
\item The special tag \texttt{MC} stands for mc-chunks, so
  \enquote{real content} (the matrix has containment rules for this
  too).

\item Currently there is only only negative number \texttt{\textminus1}
...
\item The tags \texttt{Part}, \texttt{Div} and \texttt{NoStruct} are sometimes (not always) \enquote{transparent}, that means that it depends on their parent if a child tag is allowed. The code tries to
    detect this, but  if a structure using one of these tags is stashed it can't identify the parent and so won't be able to check the rule 
\end{itemize} 

  
Parent-child checks are not fast, so there is an option to disable them
\begin{lstlisting}
 \tagpdfsetup{debug/parent-child-check=off}
\end{lstlisting}
This will be speed up the compilation but should be used with care, parent-child checks are an important tool to detect faulty structures!

They can be reenabled with
\begin{lstlisting}
 \tagpdfsetup{debug/parent-child-check=on}
\end{lstlisting}

When using lualatex there is also the value \texttt{atend}. This disables the inline checks, but at the end of the document, the Lua code will go through the structures and check the rules.  

There is also an online service \cite{showtags} which validates the structure with an Relax NG schema developed by the LaTeX Team, which can be found at \cite{showtags-script}. 
...
export to text or xml in Adobe Pro.
...
\begin{docCommands}
 {
  {doc name=tag_spacechar_off:},
  {doc name=tag_spacechar_on:}
 }
\end{docCommands}
...
\item when using the box the first time 
 ...
 \item if it has no tagging commands then reset the attribute and use 
  ...
  \begin{verbatim}
  \ExplSyntaxOn
  \let\tagmcresetbox\tag_mc_reset_box:N %make a copy 
  \ExplSyntaxOff
  ...
...
\end{itemize}
 ...
 By default (if tagging is active) \pkg{tagpdf} embeds now such CSS style definitions. 
 Currently the list of files is rather short and consists of two files (with extension
 .html and \verb+<style>...</style>+ html tags) which are provided by the \pkg{tagpdf} package:
 \begin{itemize}
 \item \texttt{latex-align-css.html} which improves the styling of \pkg{amsmath} alignments tagged with MathML.
...
\item[\PrintKeyName{css-list-add}] this key allows to add more files to the
...
\begin{docCommand}{ShowTagging}{\marg{key-val-list}}\end{docCommand}
...
\begin{description}
  ...
  new page. The value is a positive integer and sets the first mc-shown. If no value
...
\item Keys for alternative and actualtext. How to define the input encoding? Like in \pkg{accsupp}?
...
\item \textbf{More engines supported} The generic mode of \pkg{tagpdf} 
    ...
    backends don't support the |interwordspace| option. With Xe\LaTeX{} 
...
\item The commands \cs{tagpdfifluatexTF} and \cs{tagpdfifpdftexTF} have been 
    removed.
...
\item[Names] \PDF{} knows a sort of variable called a
  ...
  Quite a number of the options of \pkg{tagpdf} actually define
...
\item[Strings]There are two types of strings: \emph{Literal strings}
  ...
  ascii chars and octal numbers:
    ...
    \emph{Hexadecimal strings} are enclosed in angle brackets. They
...
\item[Arrays] Arrays are enclosed by square brackets. They can contain
  ...
  spaces \texttt{15 0 R} is \emph{one} element of the array.
    ...
    \begin{tikzpicture}[baseline=(a1.base),alt={Illustration of array}]
    \node[arg](a1) {0};
