...
the PDF management is loaded if you use the \cs{DocumentMetadata} command. 
Tagging can then be activated by using the \texttt{testphase} key. The exact behaviour of 
...
When the package is loaded it will -- apart from loading more packages and defining a lot of things -- not do much. You will have to activate it with \verb+\tagpdfsetup+ or as shown above in \verb+\DocumentMetadata+.
...
This command give back some variables. Currently the only working key words are \verb+mc_tag+, \verb+struct_tag+ and \verb+struct_num+. The last returns a number works also if only \pkg{tagpdf-base} has been loaded.
...
\begin{description}
  ...
  % By default a structure is added as kid to the currently active structure.
   ...
   \cs{ref_value:nn}\verb+{tagpdfstruct-label}{tagstruct}+ to retrieve it.
