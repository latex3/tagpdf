...
\DebugBlocksOff
\makeatletter
\def\UlrikeFischer@package@version{1.0c}
\def\UlrikeFischer@package@date{2026-05-17}
\makeatother
...
the \pkg{latex-lab} bundle (part of the core \LaTeX{} distribution),  the \pkg{pdfmanagement-testphase} 
...
\item[\PrintKeyName{activate/softhyphen}] luamode only.
  ...
  \begin{itemize}
  \item \texttt{false} and \texttt{off} deactivates the automatic handling. 
  ...
  \item \texttt{true}, \texttt{on}  do currently the same as \texttt{artifact}.
 \end{itemize}
 
  If tagging is not activate the softhyphen handling is not activated, by default and the hyphen stays a hard-hyphen. If is possible to use the \texttt{char} mode with 
  \begin{verbatim}
